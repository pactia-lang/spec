...
Only `[package]` and `[dependencies]`. No module list, no stack binding, no kind field.
...
`UNKNOWN_SYMBOL`, `DEF_IN_PRODUCT`, `DEF_PLACEMENT_REQUIRED`, `PLACEMENT_VIOLATION`, `REGISTRY_COLLISION`, `DEPENDENCY_NOT_DECLARED`, `VERSION_IN_IMPORT`, `MACRO_UNKNOWN`, `MACRO_ARGS_INVALID`, `MACRO_EXPANSION_CYCLE`, `IMPORT_UNUSED`, `ATTACH_UNDEFINED`, `ATTACH_KIND_MISMATCH`, `CONTEXT_IMPORT_UNUSED`, `CONTEXT_ATTACH_UNDEFINED`, `CONTEXT_ATTACH_KIND_MISMATCH`, `FRAGMENT_PACKAGE_IMPORT`, `TAG_BODY_MISSING_FIELD`, `TAG_BODY_UNKNOWN_FIELD`, `CLAUSE_DUPLICATE_KEY`, `PACKAGE_NOT_FOUND`, `PACKAGE_LOCK_MISMATCH`, `LOCK_ENTRY_MISSING`.
